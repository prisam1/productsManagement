...
const getProducts = async function (req, res) {
    try {
        ...
        if (size) 
             { productData.availableSizes = { $in: size.toUpperCase().split(",").map(x => x.trim()) } }
        if (priceLessThan && priceGreaterThan) 
             { productData.price = { $gt: priceGreaterThan, $lt: priceLessThan } }
        if (priceLessThan && !priceGreaterThan) 
             { productData.price = { $lt: priceLessThan } }
        if (priceGreaterThan && !priceLessThan)
             { productData.price = { $gt: priceGreaterThan } }

        if (name) {
            product = await productModel.find({ productData, title: new RegExp(name, 'i') }).sort({ price: priceSort }) 
        ...
        let productsByFilter = await productModel.find(productData).sort({ price: priceSort })
